...
import { useUIStore, type Theme } from "@/shared/model/store/ui";
...
<template>
  <div class="flex p-1 bg-[var(--surface-muted)] rounded-[var(--radius-sm)] w-full sm:w-auto overflow-hidden">
    <button
      v-for="option in options"
      :key="option.value"
      type="button"
      class="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3.5 py-2 rounded-[var(--radius-xs)] border-0 text-[11px] font-bold transition-all cursor-pointer"
