...
describe("ProfileSocialService", () => {
  ...
  it("parses only known notification metadata actions", async () => {
    const { parseNotificationMetadata } = await import("@/shared/api/services/ProfileSocialService");
  ...
  it("uses profile social endpoints without auth service coupling", async () => {
    const { profileClient } = await import("@/shared/api/client");
    const { ProfileSocialService } = await import("@/shared/api/services/ProfileSocialService");
