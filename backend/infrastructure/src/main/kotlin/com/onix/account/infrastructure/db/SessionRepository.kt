...
class SessionRepository(private val dataSource: DataSource) {
    companion object {
        ...
        private const val CREATE_SQL = """
            INSERT INTO sessions (id, user_id, refresh_token_hash, device_id, user_agent, ip_address, expires_at, active_owner_type, active_owner_id, last_used_at, created_at)
            VALUES (?::uuid, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            RETURNING id
        """
    ...
    fun create(session: Session): String {
        dataSource.connection.use { conn ->
            val id = conn.prepareStatement(CREATE_SQL).use { stmt ->
                ...
                stmt.setString(8, session.activeOwnerType)
                stmt.setObject(9, UUID.fromString(session.activeOwnerId))
                stmt.setTimestamp(10, java.sql.Timestamp.from(session.lastUsedAt))
                stmt.setTimestamp(11, java.sql.Timestamp.from(session.createdAt))
