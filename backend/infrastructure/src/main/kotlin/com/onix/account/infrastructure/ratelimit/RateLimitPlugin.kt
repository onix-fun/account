package com.onix.account.infrastructure.ratelimit
...
import com.onix.account.shared.ApiErrorResponse
...
val RateLimit = createApplicationPlugin("RateLimit", ::RateLimitConfig) {
    ...
    onCall { call ->
        ...
        val matched = rules.entries.firstOrNull { (pattern, _) ->
            requestPath.startsWith(pattern)
        }

        if (matched != null) {
            val (pattern, matchedRule) = matched
            val ipKey = call.request.origin.remoteHost
            val allowed = runCatching {
                rateLimitRepository.checkAndIncrement("ratelimit:$pattern:ip", ipKey, matchedRule.max, matchedRule.windowSeconds)
            }.getOrElse {
                call.respond(
