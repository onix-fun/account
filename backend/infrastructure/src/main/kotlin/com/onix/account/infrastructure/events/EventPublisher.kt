package com.onix.account.infrastructure.events
...
class EventPublisher(private val dataSource: DataSource, security: SecurityConfig) {
    ...
    fun publish(type: String, payload: String, expiresAt: Instant = Instant.now().plusSeconds(900)) {
        dataSource.connection.use { conn ->
            conn.prepareStatement("INSERT INTO email_outbox(id,event_type,payload,expires_at) VALUES (?,?,?,?)").use {
                it.setObject(1, UuidV7.generate()); it.setString(2, type); it.setString(3, encrypt(payload)); it.setTimestamp(4, java.sql.Timestamp.from(expiresAt)); it.executeUpdate()
            }; conn.commit()
        }
    }
