...
class OrganizationService(
    ...
    fun create(userId: String, request: CreateOrganizationRequest): OrganizationDto {
        ...
        profileClient.update("ORGANIZATION", org.id, org.orgName, displayName,
            request.bio?.trim()?.take(500).orEmpty(), request.socialLinks?.let(::normalizeSocialLinks).orEmpty())
        return org.withPublicProfile().toDto(OrganizationRole.OWNER)
    }
    ...
    fun update(userId: String, orgId: String, request: UpdateOrganizationRequest): OrganizationDto {
        ...
        val current = organizationRepository.findById(orgId)?.withPublicProfile() ?: throw IllegalArgumentException("Organization not found")
        ...
        profileClient.update("ORGANIZATION", orgId, updated.orgName, request.displayName?.trim()?.takeIf(String::isNotBlank)?.take(120) ?: current.displayName,
            if (request.bioProvided) request.bio?.trim()?.take(500).orEmpty() else current.bio.orEmpty(),
            request.socialLinks?.let(::normalizeSocialLinks) ?: current.socialLinks)
        return updated.withPublicProfile().toDto(OrganizationRole.OWNER)
    }

    fun updateAvatar(userId: String, orgId: String, bytes: ByteArray, contentType: String): OrganizationDto {
        requireRole(userId, orgId, OrganizationRole.OWNER)
        val current = organizationRepository.findById(orgId)?.withPublicProfile() ?: throw IllegalArgumentException("Organization not found")
        if (profileClient.get("ORGANIZATION", orgId) == null) {
            profileClient.update("ORGANIZATION", orgId, current.orgName, current.displayName, current.bio.orEmpty(), current.socialLinks)
        }
        val assetId = mediaClient.upload("ORGANIZATION", orgId, bytes, contentType)
        profileClient.setAvatar("ORGANIZATION", orgId, assetId)
        return current.withPublicProfile().toDto(OrganizationRole.OWNER)
    }
    ...
    fun context(userId: String, activeOwner: OwnerRef): OrganizationContextResponse {
        ...
        val organizations = organizationRepository.listForUser(userId).map { it.first.withPublicProfile() to it.second }
    ...
    fun listMine(userId: String): List<OrganizationDto> =
        organizationRepository.listForUser(userId).map { it.first.withPublicProfile().toDto(it.second) }

    fun findByName(name: String): OrganizationDto? =
        organizationRepository.findByName(name)?.withPublicProfile()?.toDto()

    fun search(query: String, limit: Int): List<OrganizationDto> =
        organizationRepository.search(query, limit).map { it.withPublicProfile().toDto() }
    ...
    fun findOwner(ref: OwnerRef): OwnerIdentityDto? =
        when (ref.type) {
            ...
            OwnerType.ORGANIZATION -> organizationRepository.findById(ref.id.toString())?.withPublicProfile()?.toOwnerIdentity()
        }
    ...
    private fun OrganizationInvitation.toDto(): OrganizationInvitationDto {
        val org = organizationRepository.findById(organizationId)?.withPublicProfile() ?: throw IllegalArgumentException("Organization not found")
    ...
    private fun Organization.withPublicProfile(): Organization {
        val profile = runCatching { profileClient.get("ORGANIZATION", id) }.getOrNull() ?: return this
        return copy(
            displayName = profile.displayName.ifBlank { orgName },
            bio = profile.bio.takeIf(String::isNotBlank),
            socialLinks = profile.socialLinks,
            avatarUrl = mediaClient.resolve("ORGANIZATION", id, profile.avatarAssetId)
        )
    }
}
