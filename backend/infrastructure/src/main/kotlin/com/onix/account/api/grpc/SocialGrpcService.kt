package com.onix.account.api.grpc

import com.onix.account.contract.*
import com.fasterxml.uuid.Generators
import io.grpc.Status
import io.grpc.stub.StreamObserver
import com.onix.account.domain.NotificationServiceCatalog
import com.onix.account.domain.NotificationTypeCatalog
import com.onix.account.domain.OwnerRef as DomainOwnerRef
import com.onix.account.domain.ProfileVisibility
import com.onix.account.domain.PublishActivityStatus
import com.onix.account.domain.SocialLink as DomainSocialLink
import com.onix.account.domain.UserActivityType as DomainUserActivityType
import com.onix.account.infrastructure.PrivacyRepo
import com.onix.account.infrastructure.SseManager
import com.onix.account.infrastructure.db.User
import com.onix.account.organizations.OrganizationService
import com.onix.account.organizations.OwnerAction as DomainOwnerAction
import com.onix.account.search.SearchService
import com.onix.account.usecases.*
import com.onix.account.users.UserService
import com.onix.account.users.toBirthdayParts
...
class SocialGrpcService(
    ...
    override fun getProfileByUsername(request: GetProfileByUsernameRequest, responseObserver: StreamObserver<AccountProfile>) =
        unary(responseObserver) {
            val viewerId = optionalPrincipal()?.userId?.let(UUID::fromString)
    ...
    override fun getOrganizationByName(request: GetOrganizationByNameRequest, responseObserver: StreamObserver<OwnerProfile>) =
        unary(responseObserver) {
            ...
            ownerProfile(
                owner = DomainOwnerRef(com.onix.account.domain.OwnerType.ORGANIZATION, UUID.fromString(org.id)),
                viewer = viewer
            )
        }
    ...
    override fun searchOwners(request: SearchUsersRequest, responseObserver: StreamObserver<OwnerListResponse>) =
        unary(responseObserver) {
            ...
            val users = searchService().searchByUsernamePrefix(request.query, request.limit.coerceIn(1, 25))
                .map { ownerProfile(DomainOwnerRef(com.onix.account.domain.OwnerType.USER, UUID.fromString(it.id)), principal.activeOwnerRef()) }
            val orgs = organizationService().search(request.query, request.limit.coerceIn(1, 25))
                .map { ownerProfile(DomainOwnerRef(com.onix.account.domain.OwnerType.ORGANIZATION, UUID.fromString(it.id)), principal.activeOwnerRef()) }
    ...
    override fun getOwnerVisibility(request: OwnerVisibilityRequest, responseObserver: StreamObserver<VisibilityResponse>) =
        unary(responseObserver) {
            val principal = optionalPrincipal()
            val viewer = request.viewer.takeIf { it.ownerId.isNotBlank() }?.toDomain() ?: principal?.activeOwnerRef()
            if (principal != null && viewer != null && viewer != principal.activeOwnerRef()) {
            ...
            VisibilityResponse.newBuilder()
                ...
                .setViewerId(viewer?.id?.toString().orEmpty())
    ...
    private fun requireUserId(fallbackWhenAuthDisabled: String? = null): String {
        if (!requireUserToken) return fallbackWhenAuthDisabled ?: Generators.timeBasedEpochGenerator().generate().toString()
    ...
    private fun requirePrincipal(fallbackWhenAuthDisabled: String? = null): GrpcPrincipal {
        if (!requireUserToken) {
            val id = fallbackWhenAuthDisabled ?: Generators.timeBasedEpochGenerator().generate().toString()
            return GrpcPrincipal(id, "", com.onix.account.domain.OwnerType.USER.name, id)
        }
    ...
    private fun User.toRelatedGrpc(relationship: com.onix.account.domain.Relationship): RelatedUser =
    ...
    private fun ownerProfile(owner: DomainOwnerRef, viewer: DomainOwnerRef?): OwnerProfile {
        ...
        val relationship = socialUseCases.getRelationship(viewer, owner)
        val isPrivate = socialUseCases.getPrivacySettings(owner).isPrivate
        val user = if (owner.type == com.onix.account.domain.OwnerType.USER) userService().getProfile(owner.id.toString()) else null
        val organization = if (owner.type == com.onix.account.domain.OwnerType.ORGANIZATION) organizationService().findByName(identity.username) else null
        val bio = when (owner.type) {
            com.onix.account.domain.OwnerType.USER -> user?.bio
            com.onix.account.domain.OwnerType.ORGANIZATION -> organization?.bio
        }.orEmpty()
        val socialLinks = when (owner.type) {
            com.onix.account.domain.OwnerType.USER -> user?.socialLinks
            com.onix.account.domain.OwnerType.ORGANIZATION -> organization?.socialLinks
        }.orEmpty()
    ...
    private fun com.onix.account.domain.OwnerIdentityDto.toGrpc(): OwnerIdentity =
    ...
    private fun com.onix.account.domain.OwnerType.toGrpc(): OwnerType =
        when (this) {
            com.onix.account.domain.OwnerType.USER -> OwnerType.USER
            com.onix.account.domain.OwnerType.ORGANIZATION -> OwnerType.ORGANIZATION
        }

    private fun OwnerRef.toDomain(): com.onix.account.domain.OwnerRef {
        val type = when (ownerType) {
            OwnerType.ORGANIZATION -> com.onix.account.domain.OwnerType.ORGANIZATION
            else -> com.onix.account.domain.OwnerType.USER
        }
        return com.onix.account.domain.OwnerRef(type, UUID.fromString(ownerId))
    }

    private fun GrpcPrincipal.activeOwnerRef(): DomainOwnerRef =
        DomainOwnerRef(
            type = runCatching { com.onix.account.domain.OwnerType.valueOf(activeOwnerType) }.getOrDefault(com.onix.account.domain.OwnerType.USER),
    ...
    private fun com.onix.account.domain.Relationship.toGrpc(): RelResponse =
    ...
    private fun LocalizedText.toDomain(): com.onix.account.domain.LocalizedText =
        com.onix.account.domain.LocalizedText(ru = ru, en = en)
}
