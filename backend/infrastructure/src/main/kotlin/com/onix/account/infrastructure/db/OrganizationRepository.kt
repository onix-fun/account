...
import com.onix.account.domain.Organization
import com.onix.account.domain.OrganizationInvitation
import com.onix.account.domain.OrganizationInvitationStatus
import com.onix.account.domain.OrganizationMember
import com.onix.account.domain.OrganizationRole
...
class OrganizationRepository(private val dataSource: DataSource) {
        ...
        dataSource.connection.use { conn ->
            try {
                val created = conn.prepareStatement("""
                    INSERT INTO organizations (id, org_name, status, created_by_user_id, created_at, updated_at)
                    VALUES (?::uuid, ?, ?, ?, ?, ?)
                    RETURNING *
                """.trimIndent()).use { stmt ->
                    stmt.setString(1, org.id)
                    stmt.setString(2, org.orgName)
                    stmt.setString(3, org.status)
                    stmt.setObject(4, UUID.fromString(org.createdByUserId))
                    stmt.setTimestamp(5, Timestamp.from(org.createdAt))
                    stmt.setTimestamp(6, Timestamp.from(org.updatedAt))
    ...
    fun update(orgId: String, orgName: String, displayName: String, bio: String?, socialLinks: List<SocialLink>, avatarUrl: String?): Organization {
        dataSource.connection.use { conn ->
            val updated = conn.prepareStatement("""
                ...
                SET org_name = ?, updated_at = ?
                WHERE id = ? AND status = 'ACTIVE'
                RETURNING *
            """.trimIndent()).use { stmt ->
                stmt.setString(1, orgName)
                stmt.setTimestamp(2, Timestamp.from(Instant.now()))
                stmt.setObject(3, UUID.fromString(orgId))
                val rs = stmt.executeQuery()
    ...
    fun createInvitation(invitation: OrganizationInvitation): OrganizationInvitation {
        dataSource.connection.use { conn ->
            val created = conn.prepareStatement("""
                INSERT INTO organization_invitations (id, organization_id, invited_user_id, invited_by_user_id, role, status, created_at, updated_at, expires_at)
                VALUES (?::uuid, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                RETURNING *
            """.trimIndent()).use { stmt ->
                stmt.setString(1, invitation.id)
                stmt.setObject(2, UUID.fromString(invitation.organizationId))
                stmt.setObject(3, UUID.fromString(invitation.invitedUserId))
                stmt.setObject(4, UUID.fromString(invitation.invitedByUserId))
                stmt.setString(5, invitation.role.name)
                stmt.setString(6, invitation.status.name)
                stmt.setTimestamp(7, Timestamp.from(invitation.createdAt))
                stmt.setTimestamp(8, Timestamp.from(invitation.updatedAt))
                if (invitation.expiresAt != null) stmt.setTimestamp(9, Timestamp.from(invitation.expiresAt)) else stmt.setTimestamp(9, null)
                val rs = stmt.executeQuery()
                rs.next()
                mapInvitation(rs)
            }
    ...
    private fun mapOrganization(rs: ResultSet) = Organization(
        ...
        displayName = rs.getString("org_name"),
        bio = null,
