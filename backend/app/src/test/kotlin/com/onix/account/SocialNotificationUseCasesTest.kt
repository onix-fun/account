...
import com.onix.account.contract.PublishUserActivityRequest
import com.onix.account.contract.UserActivityType as ProtoUserActivityType
...
import com.onix.account.domain.SubscriptionStatus
import com.onix.account.domain.UserActivityEvent
import com.onix.account.domain.UserActivityType
import com.onix.account.domain.UserBlock
import com.onix.account.domain.PrivacySettings
import com.onix.account.domain.FieldVisibility
import com.onix.account.domain.ProfileVisibility
import com.onix.account.domain.VisibilityAudience
import com.onix.account.api.grpc.SocialGrpcService
import com.onix.account.infrastructure.NotificationOutboxWorker
...
class SocialNotificationUseCasesTest {
    ...
    fun `profile field visibility respects private profile gate and friends`() {
        ...
        val follower = com.onix.account.domain.Relationship(
            isFollowing = true,
            isFollowedBy = false,
            isFriend = false,
            isBlocked = false,
    ...
    fun `grpc publish user activity returns accepted then duplicate`() {
        ...
        val first = CapturingObserver<com.onix.account.contract.PublishUserActivityResponse>()
        val second = CapturingObserver<com.onix.account.contract.PublishUserActivityResponse>()
    ...
    fun `grpc publish user activity validates source event id`() {
        ...
        val notificationUseCases = NotificationUseCases(FakeNotificationRepository(), FakeOutboxRepository())
        val service = SocialGrpcService(fixtures.socialUseCases, fixtures.blockRepo, fixtures.socialRepo, notificationUseCases)
        val observer = CapturingObserver<com.onix.account.contract.PublishUserActivityResponse>()
        val request = PublishUserActivityRequest.newBuilder()
