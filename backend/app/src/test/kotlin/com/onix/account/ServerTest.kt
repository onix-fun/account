...
import com.onix.account.app.module
...
import com.onix.account.auth.ConfirmRegistrationRequest
import com.onix.account.auth.ForgotPasswordRequest
import com.onix.account.auth.LoginRequest
import com.onix.account.auth.RegisterRequest
import com.onix.account.auth.ResetPasswordRequest
...
import com.onix.account.users.UpdateProfileRequest
...
private class AccountPostgresContainer(image: String) : PostgreSQLContainer<AccountPostgresContainer>(image)
private class AccountRedisContainer(image: DockerImageName) : GenericContainer<AccountRedisContainer>(image)

class ServerTest {

    private fun TestApplicationBuilder.setupTestConfig(cookieDomain: String? = null, cookieSecure: Boolean = false) {
        val dbUrl = System.getenv("POSTGRES_URL") ?: postgres.jdbcUrl
        val dbUser = System.getenv("POSTGRES_USER") ?: postgres.username
        val dbPass = System.getenv("POSTGRES_PASSWORD") ?: postgres.password
        val redisUrl = System.getenv("REDIS_URL") ?: "redis://${redis.host}:${redis.getMappedPort(6379)}"
        ...
        environment {
            config = MapApplicationConfig(
                ...
                "identity.jwt.issuer" to "account",
                ...
                "downstream.profile.target" to "localhost:19092",
                "downstream.profile.api_key" to "profile-secret",
                "downstream.media.target" to "localhost:19093",
                "downstream.media.api_key" to "media-secret",
    ...
    private fun codeForPendingRegistration(store: PendingRegistrationStore, email: String): String {
        checkNotNull(store.findByEmail(email)) { "Pending registration missing for $email" }
        val field = PendingRegistrationStore::class.java.getDeclaredField("dataSource").apply { isAccessible = true }
        return latestEmailCode(field.get(store) as javax.sql.DataSource, "email.verify", email)
    }
    ...
    private fun codeForVerificationToken(repository: VerificationTokenRepository, purpose: String): String {
        ...
        checkNotNull(repository.latest(userId, purpose)) { "Missing token" }
        ...
        val email = dataSource.connection.use { conn ->
            conn.prepareStatement("SELECT email FROM users WHERE id=?::uuid").use {
                it.setString(1, userId)
                val rs = it.executeQuery()
                if (rs.next()) rs.getString(1) else error("Missing user for verification token")
            }
        }
        val eventType = if (purpose == "PASSWORD_RESET") "email.password_reset" else "email.verify"
        return latestEmailCode(dataSource, eventType, email)
    }

    private fun latestEmailCode(dataSource: javax.sql.DataSource, eventType: String, email: String): String {
        val publisher = com.onix.account.infrastructure.events.EventPublisher(
            dataSource,
            com.onix.account.infrastructure.config.SecurityConfig(
                TEST_OTP_SECRET,
        ...
        return dataSource.connection.use { conn ->
            conn.prepareStatement("SELECT payload FROM email_outbox WHERE event_type=? ORDER BY created_at DESC LIMIT 20").use {
                it.setString(1, eventType)
                val rows = it.executeQuery()
                while (rows.next()) {
                    val payload = Json.parseToJsonElement(publisher.decrypt(rows.getString(1))).jsonObject
                    if (payload["email"]?.toString()?.trim('"') == email) {
                        return@use payload["code"]?.toString()?.trim('"') ?: error("Email code is missing")
                    }
                }
                error("Email code is missing for $email")
            }
        }
    }
    ...
    private companion object {
        ...
        private const val TEST_INTERNAL_SECRET = "test-internal-auth-secret-at-least-32-characters"
        private val postgres = AccountPostgresContainer("postgres:18-alpine").apply { start() }
        private val redis = AccountRedisContainer(DockerImageName.parse("redis:8.2.6-alpine"))
