...
import com.onix.account.infrastructure.di.koinModule
import com.onix.account.infrastructure.events.EmailEventConsumer
import com.onix.account.infrastructure.jwt.RsaKeyLoader
import com.onix.account.infrastructure.ratelimit.RateLimit
...
import com.onix.account.search.*
import com.onix.account.sessions.*
import com.onix.account.organizations.*
import com.onix.account.socialModule
import com.onix.account.api.grpc.SocialGrpcService
import com.onix.account.api.rest.*
import com.onix.account.users.*
...
fun Application.module() {
    ...
    intercept(ApplicationCallPipeline.Setup) {
        ...
        if (contentLength != null && contentLength > MAX_BODY_SIZE) {
            context.respond(HttpStatusCode.PayloadTooLarge, com.onix.account.shared.ApiErrorResponse(
                code = com.onix.account.shared.ApiErrorCode.AVATAR_TOO_LARGE.name,
                numericCode = com.onix.account.shared.ApiErrorCode.AVATAR_TOO_LARGE.numericCode,
                message = com.onix.account.shared.ApiErrorCode.AVATAR_TOO_LARGE.message,
    ...
    install(RequestValidation) {
        validate<RegisterRequest> { request ->
            when {
                request.email.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "email")
                !request.email.contains("@") -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_EMAIL, "email")
                request.username.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "username")
                request.username.trim().length < 3 -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_USERNAME_TOO_SHORT, "username")
                request.password.length < 8 -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_PASSWORD_TOO_SHORT, "password")
        ...
        validate<ConfirmRegistrationRequest> { request ->
            ...
            when {
                identifier.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "identifier")
                !request.code.matches(Regex("\\d{6}")) -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_CODE, "code")
                else -> ValidationResult.Valid
            }
        }
        ...
        validate<LoginRequest> { request ->
            ...
            when {
                identifier.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "identifier")
                request.password.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "password")
                else -> ValidationResult.Valid
            }
        }
        ...
        validate<ResetPasswordRequest> { request ->
            ...
            when {
                identifier.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "identifier")
                !code.matches(Regex("\\d{6}")) -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_CODE, "code")
                request.newPassword.length < 8 -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_PASSWORD_TOO_SHORT, "newPassword")
                else -> ValidationResult.Valid
            }
        }
        validate<ChangePasswordRequest> { request ->
            when {
                request.currentPassword.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "currentPassword")
                request.newPassword.length < 8 -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_PASSWORD_TOO_SHORT, "newPassword")
                else -> ValidationResult.Valid
            }
        }
        ...
        validate<PublicVerificationRequest> { request ->
            if (request.identifier.isBlank()) invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "identifier")
            else ValidationResult.Valid
        }
        validate<PublicVerificationConfirmRequest> { request ->
            when {
                request.identifier.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "identifier")
                !request.code.matches(Regex("\\d{6}")) -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_CODE, "code")
                else -> ValidationResult.Valid
            }
        }
        validate<QrLoginConsumeRequest> { request ->
            ...
            when {
                hasScanToken == hasManualCode -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "scanToken,manualCode")
                hasManualCode && request.manualCode.orEmpty().filter(Char::isLetterOrDigit).length != 12 ->
                    invalid(com.onix.account.shared.ApiErrorCode.AUTH_QR_CODE_INVALID, "manualCode")
                else -> ValidationResult.Valid
            }
        }
        validate<RequestEmailChangeRequest> { request ->
            when {
                request.currentPassword.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "currentPassword")
                request.newEmail.isBlank() -> invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "newEmail")
                else -> ValidationResult.Valid
            }
        }
        validate<ConfirmEmailChangeRequest> { request ->
            if (!request.code.matches(Regex("\\d{6}"))) invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_CODE, "code")
            else ValidationResult.Valid
        }
        validate<UpdateProfileRequest> { request ->
            ...
            when {
                u != null && u.isBlank() ->
                    invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_REQUIRED_FIELD, "username")
                u != null && u.trim().length < 3 ->
                    invalid(com.onix.account.shared.ApiErrorCode.VALIDATION_USERNAME_TOO_SHORT, "username")
                else -> ValidationResult.Valid
            }
        }
    }
        ...
        if (method in setOf(HttpMethod.Post, HttpMethod.Put, HttpMethod.Patch, HttpMethod.Delete)) {
            ...
            if (!isCsrfExemptPath && !hasBearer) {
                ...
                if (headerToken.isBlank() || cookieToken.isBlank() || !java.security.MessageDigest.isEqual(headerToken.toByteArray(), cookieToken.toByteArray())) {
                    call.respondApiError(com.onix.account.shared.ApiErrorCode.SECURITY_CSRF_INVALID)
                    return@intercept
                }
            }
        }
    ...
    install(StatusPages) {
        exception<RequestValidationException> { call, cause ->
            ...
            val error = parts.firstOrNull()?.let { runCatching { com.onix.account.shared.ApiErrorCode.valueOf(it) }.getOrNull() }
                ?: com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_REQUEST
            val fields = parts.getOrNull(1)?.split(",")?.filter { it.isNotBlank() }.orEmpty()
            call.respond(error.status, com.onix.account.shared.ApiErrorResponse(error.name, error.numericCode, error.message, fields.map { com.onix.account.shared.ApiFieldError(it, error.name, error.numericCode) }))
        }
        exception<com.onix.account.shared.ApiException> { call, cause ->
            call.respond(cause.error.status, com.onix.account.shared.ApiErrorResponse(cause.error.name, cause.error.numericCode, cause.error.message, cause.fields.map { com.onix.account.shared.ApiFieldError(it, cause.error.name, cause.error.numericCode) }))
        }
        exception<UnauthorizedException> { call, _ ->
            val error = com.onix.account.shared.ApiErrorCode.AUTH_UNAUTHORIZED
            call.respond(error.status, com.onix.account.shared.ApiErrorResponse(error.name, error.numericCode, error.message))
        }
        exception<IllegalArgumentException> { call, cause ->
            call.application.log.warn("Invalid request: ${cause.message}")
            val error = com.onix.account.shared.ApiErrorCode.VALIDATION_INVALID_REQUEST
            call.respond(error.status, com.onix.account.shared.ApiErrorResponse(error.name, error.numericCode, cause.message ?: error.message))
        }
        exception<IllegalStateException> { call, cause ->
            call.application.log.error("Illegal state: ${cause.message}", cause)
            val error = com.onix.account.shared.ApiErrorCode.INFRASTRUCTURE_SERVICE_UNAVAILABLE
            call.respond(error.status, com.onix.account.shared.ApiErrorResponse(error.name, error.numericCode, error.message))
        }
        exception<Throwable> { call, cause ->
            call.application.log.error("Internal server error: ${cause.message}", cause)
            val error = com.onix.account.shared.ApiErrorCode.INFRASTRUCTURE_INTERNAL_ERROR
            call.respond(error.status, com.onix.account.shared.ApiErrorResponse(error.name, error.numericCode, error.message))
        }
    }
    ...
    val searchService by inject<com.onix.account.search.SearchService>()
    ...
    val profileClient by inject<com.onix.account.infrastructure.clients.ProfileClient>()
    val mediaAvatarClient by inject<com.onix.account.infrastructure.clients.MediaAvatarClient>()
    environment.monitor.subscribe(ApplicationStopping) {
        profileClient.close()
        mediaAvatarClient.close()
    }
    val socialGrpcService by inject<SocialGrpcService>()
    val socialUseCases by inject<com.onix.account.usecases.SocialUseCases>()
    val notificationUseCases by inject<com.onix.account.usecases.NotificationUseCases>()
    val sseManager by inject<com.onix.account.infrastructure.SseManager>()
    val eventBus by inject<com.onix.account.infrastructure.EventBus>()
    val notificationOutboxWorker by inject<com.onix.account.infrastructure.NotificationOutboxWorker>()
    val birthdayNotificationService by inject<com.onix.account.usecases.BirthdayNotificationService>()
    val socialRepo by inject<com.onix.account.infrastructure.SocialRepo>()
    val privacyRepo by inject<com.onix.account.infrastructure.PrivacyRepo>()
    val notificationRepo by inject<com.onix.account.infrastructure.NotificationRepo>()
...
private fun invalid(error: com.onix.account.shared.ApiErrorCode, vararg fields: String): ValidationResult.Invalid {
    return ValidationResult.Invalid("${error.name}|${fields.joinToString(",")}")
}

private suspend fun io.ktor.server.application.ApplicationCall.respondApiError(error: com.onix.account.shared.ApiErrorCode, fields: List<String> = emptyList()) {
    respond(
        error.status,
        com.onix.account.shared.ApiErrorResponse(
            code = error.name,
            numericCode = error.numericCode,
            message = error.message,
            fieldErrors = fields.map { com.onix.account.shared.ApiFieldError(it, error.name, error.numericCode) }
        )
    )
}
